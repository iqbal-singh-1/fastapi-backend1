...
def get_bids(
    ...
    print(f"Current Farmer ID: {current_user.id}")  # Debug print
    bids = db.query(Bid).join(Product).filter(
        Product.farmer_id == current_user.id).all()

    print("Fetched Bids: ", bids)  # Debug print
...
def update_bid_status(
    ...
    bid = db.query(Bid).filter(Bid.id == bid_id).join(
        Product).filter(Product.farmer_id == current_user.id).first()
    if not bid:
